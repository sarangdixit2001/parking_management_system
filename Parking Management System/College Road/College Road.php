...
<body>
  ...
  if (isset($_POST['submit'])) {
    ...
    $vehiclequery = " select * from college_slot_bookings where vehicle = '$vehicle'";
    $query = mysqli_query($con, $vehiclequery);

    $slotcount = mysqli_num_rows($query);

    $hourquery = " select * from college_slot_bookings where hour = '$hour'";
    $hquery = mysqli_query($con, $hourquery);

    $hourcount = mysqli_num_rows($hquery);


    $datequery = " select * from college_slot_bookings where date = '$date'";
    $dquery = mysqli_query($con, $datequery);

    $datecount = mysqli_num_rows($dquery);


    $timequery = " select * from college_slot_bookings where start_time = '$start_time'";
  ...
  ?>
      ...
      $insertquery = "INSERT into college_slot_bookings (`name`,`email`,`phnumber`,`location`,`date`, `start_time`, `vehicle`, `hour`) values('$name','$email','$phnumber','$location','$date','$start_time','$vehicle','$hour')";
      ...
      ?>
        <script>
          window.location.href = "./college_road_slot.php";
        </script>
      <?php
      } else {
      ?>
      ...
      <div class="book">
        <div class="book__form">
          <form action=" <?php echo htmlentities($_SERVER['PHP_SELF']); ?>" method="POST">
            ...
            <h2 class="heading">
            <i class="bi bi-check2-circle"></i> College Rd Check Slot Availability
            </h2>
            ...
            <div>
              ...
              <input type="hidden" name="location" value="College Road" >
            </div>
